                  ...
                  <?php $no = 1; foreach($carousels->result() as $carousel) { ?>
                    <tr>
                      ...
                      <td>
                        ...
                        <a href="<?=base_url();?>Admin/delete_carousel/<?=$carousel->id_image_carousel;?>/<?=$carousel->image;?>" class="btn btn-sm btn-danger fa fa-trash" onclick="return confirm('Apakah Anda yakin ingin menghapus data ini?')"></a>
                      </td>
                    </tr>
                  <?php } ?>
                </tbody>
...
<div class="modal fade" id="modal_add_carousel" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel1">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <form action="<?=base_url();?>Admin/save_add_carousel" method="post" enctype="multipart/form-data">
...
<script>
  ...
  $(document).ready(function(e) {
    $('#form_edit_carousel').on('submit', (function(e) {
      e.preventDefault();
      $.ajax({
        url: '<?= base_url(); ?>Admin/save_edit_carousel',
        type: 'POST',
