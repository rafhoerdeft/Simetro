                  ...
                  <?php $no = 1; foreach($pengumumans->result() as $pengumuman) { ?>
                    <tr>
                      ...
                      <td>
                        ...
                        <a href="<?=base_url();?>Admin/delete_pengumuman/<?=$pengumuman->id_pengumuman;?>" class="btn btn-sm btn-danger fa fa-trash" onclick="return confirm('Apakah Anda yakin ingin menghapus data ini?')"></a>
                      </td>
                    </tr>
                  <?php } ?>
                </tbody>
...
<div class="modal fade" id="modal_add_pengumuman" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel1">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      <form action="<?=base_url();?>Admin/save_add_pengumuman" method="post" enctype="multipart/form-data">
...
<script>
  $(document).on('click', "#mek", function() {
    ...
    $('#body_mek .mek_tanggal').datepicker('setDate', tanggal);
  ...
  $(document).ready(function(e) {
    $('#form_edit_pengumuman').on('submit', (function(e) {
      e.preventDefault();
      $.ajax({
        url: '<?= base_url(); ?>Admin/save_edit_pengumuman',
        type: 'POST',
